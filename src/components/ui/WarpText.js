...
void main() {
  ...
  float hover = uPointerActive;

  vec2 drift = vec2(time * 0.055, -time * 0.045);
  float n1 = fbm(uv * scale * 3.1 + drift);
  float n2 = fbm((uv + 19.17) * scale * 3.4 - drift.yx);
  vec2 ambient = (vec2(n1, n2) - 0.5) * uWarpStrength * 0.045 * uMotion * hover;
  ...
  float lens = smoothstep(radius, 0.0, dist) * hover;
  float bulge = t * (1.0 - t) * (1.0 - t) * 6.75 * hover;
  ...
  vec2 split = splitDir * uRefraction * 0.16 * (0.35 + lens * 1.65) * hover;
...
const drawLine = (ctx, line, x, y, letterSpacing, align) => {
  const chars = Array.from(line);
  const total = measureLine(ctx, line, letterSpacing);
...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  const maxWidth = width * (align === 'center' ? 0.86 : 0.98);
...
const WarpText = ({
  ...
  const propsRef = useRef({
    text, color, fontSize, fontWeight, fontFamily, letterSpacing, lineHeight, align,
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WarpText: WebGL unavailable.', error);
      return undefined;
    }
    ...
    const loop = (now) => {
      ...
      pointer.active += (pointer.activeTarget - pointer.active) * 0.14;
